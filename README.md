...
- **Jobs & Dispatch:** One owner/office operating workspace at `/jobs`. Views are tabs over the same job records: Dispatch Board (default), All Jobs, Waiting, Estimates, Completed, and Needs Attention. `/dispatch` redirects to `/jobs?view=dispatch`. Every job opens the same Job 360. Dispatch Board: technician lanes, unassigned queue, drag-and-drop assignment, job lock, and **Optimize Route**. Route math comes from Google Directions when `GOOGLE_MAPS_API_KEY` (or `GOOGLE_ROUTES_API_KEY`) is set on the server. Missing credentials show “Route optimization is not configured.” Preview never applies until the dispatcher clicks Apply.
