...
- **Role workspaces:** One platform, one database. After login, users land on the experience they can use: Owner/Admin/Manager → Command Center (`/dashboard`), Dispatcher → Jobs & Dispatch (`/jobs?view=dispatch`), Office/CSR → Customer Hub (`/office`), Technician/Installer → Field (`/tech`). Users with more than one authorized workspace switch without signing out. Technicians cannot open owner, dispatch, or CSR workspaces. Marketing stays a module inside the Owner Hub — there is no separate Marketing dashboard.
- **Sidebar:** Daily workspaces stay visible (Home, Dispatch, Customer Hub, Intelligence, Action Center, Inbox). Deeper destinations sit under accordion groups — Operations, Marketing, Money, Team — filtered by `can(role, permission)`. All existing routes remain.
- **Jobs & Dispatch:** One owner/office operating workspace at `/jobs`. Today / Map / Split over the same jobs. Dispatch 360 is a job drawer with Smart Match, not a permanent intelligence panel. Ask ContractorYou is a compact control. Property geocodes are cached. Technician location is permission-based. Smart Dispatch ranks eligible technicians using Technician Intelligence plus drive time when Google routing is configured. Set `GOOGLE_MAPS_SERVER_API_KEY` (server) and `GOOGLE_MAPS_BROWSER_API_KEY` (maps). Legacy `GOOGLE_MAPS_API_KEY` still works server-side. Missing keys keep manual dispatch working. Whole-day Google Route Optimization is not auto-applied in this pass.
- **Customer Hub:** Fast customer search, quick customer/job create, and send-to-Dispatch on the same Job record. Hub stays the office's daily workspace.
- **Customer 360:** `/customers/[id]` (same dossier at `/office/customers/[id]`) is the complete customer + property record: header, property hero, equipment, active work, money, communications, photos, timeline, and Ask ContractorYou. Property enrichment is a provider abstraction — no paid lookup is connected unless credentials exist. Job photos attach to company → customer → property → job through the existing upload path, with camera capture on phones. Summit can seed synthetic demo property facts; 865 HVAC records are never rewritten for the demo.
...
- **Intelligence:** `/intelligence` is the brain of the company — owner brief, Ask ContractorYou, what ContractorYou noticed, summarized opportunities, risks, recommendations, goals, and recent AI actions. It explains the same Command Center Business Health score. It does not dump the Action Center list. Configure rules, goals, automations, and permissions in **AI Control Center** (`/settings/intelligence`). Ask retrieves tenant-scoped tools, then optionally explains with OpenAI (`gpt-4o-mini`). Never invents numbers. Set `OPENAI_API_KEY` on Railway for language-model wording.
- **Action Engine:** Ask ContractorYou can read, draft, and propose work. The model may only request registered actions. ContractorYou validates, requires approval for anything with a side effect, executes through HighLevel/CommunicationProvider, and writes an audit log. Demo tenants simulate outbound work and never send real messages. Action Center is `/actions`.
- **HighLevel:** Settings → HighLevel. Preferred infrastructure for phone, SMS, conversations, leads, marketing automation, and Social Planner. After a location is connected, **Sync communications** fetches existing HighLevel conversations (read-only). Social shows accounts actually connected in HighLevel — not fake ContractorYou OAuth. Marketplace OAuth is the production path. A location Private Integration Token is testing / single-location only. Direct Google/Twilio/Resend foundations remain. Connecting HighLevel does not text customers, publish posts, or change historical jobs.
- **AI Receptionist V2:** Settings → HighLevel. HighLevel remains the SMS transport. ContractorYou tools and the scheduling state machine remain the source of truth. The receptionist classifies and replies from bounded conversation history — not the latest SMS alone — so pronouns and short replies stay on the current service concern. A real service problem can offer a visit; “yes please” starts the deterministic scheduling workflow. Informational questions do not auto-schedule. Default mode is **HighLevel Regina** (production unchanged). **ContractorYou shadow** processes the same inbound SMS, stores the proposed reply, and never sends or books. Live ContractorYou AI only sends when conversation ownership is ContractorYou. Set `OPENAI_API_KEY` on the server for language-model wording; without it, a labeled deterministic fallback is used.
- **Waiting Board:** Operations → Waiting Board (`/operations/waiting`). Jobs that are temporarily stalled — parts, warranty, customer, approval, third party, or ready to schedule. KPI numbers are operational shortcuts: Ready to Schedule, Waiting on Parts, Overdue, and Updates Due Today open a focused list so the office does not hunt across the Kanban. Ready cards use **Schedule Now** on the existing Job 360 scheduler; a saved appointment resolves the active Ready record and the Action Center item without deleting history. Dragging a card is a real workflow transition: it can stop recurring texts, notify the customer through HighLevel, create a scheduling task, and write audit history. Automatic customer updates use the existing HighLevel path only. Failed or blocked sends are never marked sent. Company defaults live at Settings → Waiting Board. Recurring updates run server-side (`GET/POST /api/cron/waiting` with `CRON_SECRET`, and again when the board loads). Intelligence can read waiting data; it cannot change waiting state.
- **Scheduling & Capacity:** Settings → Scheduling & Capacity. Compact operational cards on top; appointment windows, weekly technician availability, eligibility, auto-booking rules, and schedule exceptions edit in drawers. The scheduling engine underneath is unchanged. HighLevel only carries the conversation. Inbound SMS can create a real job on Dispatch when company rules allow it. Confirmation texts use the same canonical HighLevel sender and thread. Auto-booking defaults **off**. Maintenance visits are generated from active membership plans — not inferred from text history. Workspace: `/maintenance`.
- **Home:** `/dashboard` is the command surface: Command Hero (today’s jobs, in progress, waiting, completed), Needs You (top Action Center items), Ask ContractorYou, and one Business Snapshot. It does not restore the old 12-module warehouse. Numbers open the records they represent.
- **Global search:** The top search is tenant-scoped typeahead across customers, jobs, estimates, invoices, and matching properties. Type two or more characters to see grouped results. Clicking a result opens the existing 360/detail page. It does not load the full customer directory into the browser.
...
- **Data & Imports:** Settings → Data & Imports. Preview-first import center. ContractorYou stays the live operating system. QuickBooks is the accounting/financial history source. Housecall Pro is historical service only. Every file import still scans, matches, previews, then writes. Historical and reference records are excluded from Dispatch, Ready to Invoice, Billing Watchdog, today’s job counts, and current A/R. Owner/admin can dry-run a Housecall Pro import reset or a QuickBooks historical import reset; each requires its own typed confirmation before any delete. The QuickBooks reset removes only records created by the QuickBooks historical importer (`sourceSystem = QUICKBOOKS` and `importMode` HISTORICAL/REFERENCE). It does not disconnect QuickBooks, delete live mappings, or remove native ContractorYou / Stripe / HighLevel data. Unknown-provenance records are not deleted.
...
- **QuickBooks Online:** Settings → QuickBooks. Real Intuit OAuth. Each ContractorYou company connects its own QuickBooks company. Tokens encrypted at rest. First connection is **safe mode**: scan/preview only. Automatic sync stays off until the setup wizard is confirmed. **Sync Center** (`/settings/quickbooks/manage`) analyzes live QuickBooks data without writing to Intuit, matches customers with EXACT / HIGH / POSSIBLE / NONE confidence, and imports approved records in dependency-safe stages (customers → vendors/items/accounts → invoices → payments → expenses). Possible matches never merge silently. Re-imports are idempotent via realm-scoped QuickBooks IDs. ContractorYou → QuickBooks write-back is not enabled in this phase. The Production Data Preview remains read-only. Historical imports never auto-sync. ContractorYou remains operational truth; QuickBooks remains the accounting ledger. Webhooks stay inactive unless `INTUIT_WEBHOOK_VERIFIER_TOKEN` is set.
...
### Summit Home Services inbound-call test (human)

Do **not** reuse the 865 HVAC location, token, or phone numbers on Summit.
...
| `QUICKBOOKS_CLIENT_ID` | No | Optional server-wide Intuit Client ID. Owners can also paste keys on Settings → QuickBooks. |
