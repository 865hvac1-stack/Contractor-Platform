...
- **Scheduling & Capacity:** Settings → Scheduling & Capacity. ContractorYou owns appointment windows, technician weekly availability, date overrides, capacity, eligibility, and auto-booking. HighLevel only carries the conversation. Inbound SMS can create a real job on Dispatch when company rules allow it. Confirmation texts use the same canonical HighLevel sender and thread. Auto-booking defaults **off**. Maintenance visits are generated from active membership plans — not inferred from text history. Workspace: `/maintenance`.
