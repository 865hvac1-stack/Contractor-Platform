...
- **AI Receptionist V2:** Settings → HighLevel. HighLevel remains the SMS transport. ContractorYou tools and the scheduling state machine remain the source of truth. Default mode is **HighLevel Regina** (production unchanged). **ContractorYou shadow** processes the same inbound SMS, stores the proposed reply, and never sends or books. Live ContractorYou AI only sends when conversation ownership is ContractorYou. Set `OPENAI_API_KEY` on the server for language-model wording; without it, a labeled deterministic fallback is used.
