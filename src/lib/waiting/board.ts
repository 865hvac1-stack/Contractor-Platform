  ...
  const records = await prisma.waitingRecord.findMany({
    ...
    include: {
      ...
      communications: {
        ...
        take: 1,
        select: { failedAt: true, attemptedAt: true, provider: true, failureReason: true },
      },
    },
  ...
  const cards: WaitingCard[] = records
    .map((record) => {
      ...
      return {
        ...
        lastFailedAt: record.communications[0]?.failedAt ?? record.communications[0]?.attemptedAt ?? null,
        lastProvider: record.communications[0]?.provider ?? null,
