...
export async function validateStageOneImportPlan(
  ...
  const [analysis, total, link, create, ignore, unresolved, staleReReview] = await Promise.all([
    ...
    prisma.quickBooksImportReview.count({
      where: { ...base, status: "IGNORED", proposedAction: { in: ["IGNORE", "MARK_INACTIVE"] } },
    }),
    prisma.quickBooksImportReview.count({ where: { ...base, status: { in: UNRESOLVED } } }),
    prisma.quickBooksImportReview.count({ where: { ...base, status: "RE_REVIEW_REQUIRED" } }),
  ]);

  const expectedTotal = analysis?.categories[0]?.availableInQbo ?? null;
  const otherResolved = Math.max(0, total - link - create - ignore - unresolved);
  ...
  if (expectedTotal == null) {
    ...
    issues.push(`${expectedTotal.toLocaleString()} QuickBooks customers were analyzed, but ${total.toLocaleString()} customer plans exist.`);
  }
  if (unresolved) issues.push(`${unresolved.toLocaleString()} customer plans remain unresolved.`);
  if (staleReReview) {
    issues.push(`${staleReReview.toLocaleString()} decisions need re-review because source or candidate data changed.`);
  }
  if (totalResolved + unresolved !== total) {
    issues.push("Resolved and unresolved plan totals do not reconcile.");
  }

  const [linkTargets, createIds, decisionGroups] = await Promise.all([
    prisma.quickBooksImportReview.findMany({
      where: { ...base, status: "APPROVED", proposedAction: "LINK" },
      select: { proposedInternalId: true },
    }),
    prisma.quickBooksImportReview.findMany({
      where: { ...base, status: "APPROVED", proposedAction: "CREATE" },
      select: { quickbooksId: true },
    }),
    prisma.quickBooksReviewDecision.groupBy({
      by: ["reviewId"],
      where: {
        companyId: scope.companyId,
        environment: scope.environment,
  ...
  const targetIds = [...new Set(linkTargets.map((row) => row.proposedInternalId).filter((id): id is string => Boolean(id)))];
  const missingTargetReferences = linkTargets.filter((row) => !row.proposedInternalId).length;
  const validTargets = targetIds.length
    ? await prisma.customer.count({ where: { companyId: scope.companyId, id: { in: targetIds } } })
    : 0;
  if (missingTargetReferences || validTargets !== targetIds.length) {
    issues.push(
      `${(missingTargetReferences + (targetIds.length - validTargets)).toLocaleString()} link plans reference a missing ContractorYou customer.`
    );
  }

  const duplicateDecisions = decisionGroups.filter((row) => row._count._all > 1).length;
  if (duplicateDecisions) {
    issues.push(`${duplicateDecisions.toLocaleString()} reviews have multiple active human decisions.`);
  }

  const createQuickBooksIds = createIds.map((row) => row.quickbooksId);
  if (createQuickBooksIds.length) {
    const [mappedCollisions, customerCollisions] = await Promise.all([
      prisma.quickBooksMapping.count({
        where: {
          companyId: scope.companyId,
          environment: scope.environment,
          realmId: scope.realmId,
          entityType: "CUSTOMER",
          quickbooksId: { in: createQuickBooksIds },
        },
      }),
    ...
    const collisions = Math.max(mappedCollisions, customerCollisions);
    if (collisions) {
      issues.push(`${collisions.toLocaleString()} create-new plans already have an approved customer mapping.`);
    }
  }
...
export function safeQuickBooksErrorText(error: unknown, fallback: string) {
...
export async function loadStageOneImportPreviewSafe(
  ...
  try {
    ...
    return {
      ok: false as const,
      error: safeQuickBooksErrorText(error, "Stage 1 plan verification failed."),
    };
  }
}

export async function loadQuickBooksExceptionReviewSafe(
  prisma: PrismaClient,
  scope: QuickBooksScope,
  input: { reason?: string | null; skipped?: boolean; customerSearch?: string | null } = {}
) {
  try {
    return { ok: true as const, data: await loadQuickBooksExceptionReview(prisma, scope, input) };
  } catch (error) {
    console.error("[quickbooks-exception-review] load failed", error);
    return {
      ok: false as const,
      error: safeQuickBooksErrorText(error, "Exception queue could not be loaded."),
    };
  }
}
