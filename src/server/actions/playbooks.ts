  ...
  try {
    ...
    if (job.projectId) {
      ...
      await prisma.projectActivity.create({
        data: { companyId: ctx.company.id, projectId: job.projectId, phaseId: job.projectPhaseId, actorId: ctx.user.id, event: "VISIT_COMPLETED", summary: `${job.projectVisitPurpose || job.jobNumber} completed`, details: { jobId: job.id } },
      });
