...
describe("dispatch V2 helpers", () => {
  ...
  it("keeps Summit seed and 865 HVAC isolated in source", () => {
    ...
    expect(board).not.toContain("DispatchIntelligencePanel");
    expect(board).toContain('layout === item');
    ...
    expect(drawer).toContain("SmartMatchPanel");
    ...
    expect(ask).toContain("Ask ContractorYou");
    expect(ask).toContain("fixed right-4 bottom-4");
    ...
    expect(existsSync(resolve("src/components/workspace-switcher.tsx"))).toBe(false);
